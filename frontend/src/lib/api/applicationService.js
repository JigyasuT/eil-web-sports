...
const getAllApplications = async () => {
  try {
    ...
    if (!isServerRunning) {
      throw new Error(
        "Backend server is not running. Please start the server at localhost:7777.",
      );
    }
...
const uploadDocuments = async (
  ...
  try {
    ...
    console.log(
      `Documents to upload:`,
      Array.isArray(documents) ? `${documents.length} files` : "1 file",
    );
    ...
    if (Array.isArray(documents)) {
      ...
      documents.forEach((doc, index) => {
        console.log(
          `Appending file ${index + 1}/${documents.length}: ${doc.name}, size: ${doc.size} bytes, type: ${doc.type}`,
        );
      ...
      // Add single file
      console.log(
        `Appending single file: ${documents.name}, size: ${documents.size} bytes, type: ${documents.type}`,
      );
...
const updateApplicationStatus = async (id, status, comments = "") => {
  try {
    console.log(
      `Updating application ${id} to status: ${status} with comment: ${comments}`,
    );
    ...
    if (!["pending", "under_review", "approved", "rejected"].includes(status)) {
      throw new Error(
        `Invalid status: ${status}. Status must be one of: pending, under_review, approved, rejected`,
      );
    }
    ...
    if (error.response?.status === 403) {
      throw new Error(
        "You are not authorized to update application status. Admin access required.",
      );
    } else if (error.response?.status === 404) {
      throw new Error(
        "Application not found. It may have been deleted or moved.",
      );
