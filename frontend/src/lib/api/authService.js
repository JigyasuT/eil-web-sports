...
//     } else {
//       console.error(
//         `Error setting up request to ${error.config?.url}:`,
//         error.message,
//       );
//     }
//     return Promise.reject(error);
...
//         success: false,
//         error:
//           "CORS error: The server is not configured to accept requests from this origin. Please check server CORS settings.",
//       };
//     }
...
//         success: false,
//         error:
//           "No response from server. Please try again later or check if the server is running.",
//       };
//     }
...
//         success: false,
//         error:
//           "No response from server. Please check if the server is running at http://localhost:7777.",
//       };
//     }
...
const api = axios.create({
  ...
  timeout: 10000,
  withCredentials: false, // Change to true if using cookies with credentials
});
...
const register = async (userData) => {
  try {
    ...
    return handleRequestError(error, "Registration failed");
  }
};
...
const login = async (email, password) => {
  try {
    ...
    return handleRequestError(error, "Login failed");
  }
};
...
const getCurrentUser = async () => {
  try {
    ...
    return handleRequestError(error, "Failed to fetch user data");
  }
};
...
function ensureAdminRole() {
  try {
    ...
    const user = JSON.parse(storedUser);
    const isAdmin =
      ["admin@example.com", "admin@eil.com", "shreyans.jaiswal704@gmail.com"].includes(user.email) ||
      user.email?.toLowerCase().includes("admin");
...
function handleRequestError(error, fallbackMessage) {
  if (error.response) {
    ...
    return {
      success: false,
      error: "No response from server. Is it running at localhost:7777?",
    };
  }
  return {
    success: false,
